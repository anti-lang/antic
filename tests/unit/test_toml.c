...
static struct anti_toml *read_toml(const char *text)
{
...
static void inline_table(void)
{
    ...
    struct anti_toml *doc = read_toml(text);
...
{
    struct anti_toml *doc = read_toml("a = { b = 1\n");
...
{
    struct anti_toml *doc = read_toml("[[t]]\nx = 1\n[[t]]\nx = 2\n");
