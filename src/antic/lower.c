...
static struct ir_operand lower_simd_binary(struct lowerer *l,
                                           const struct expr *e);
static struct ir_operand lower_simd_unary(struct lowerer *l,
                                          const struct expr *e);
static struct ir_operand lower_simd_cast(struct lowerer *l,
                                         const struct expr *e);
...
{
    return temp(l, ir_entry_slot(l->f, vtype_of(l, t)));
}
...
{
    return lane->kind == TYPE_F16 ? IR_F32 : ir_type_of(lane);
}
                        ...
                        bool loop)
{
    ...
    ir_assign(l->f, l->b, c->index, zero());
    l->loop_depth++;
    c->head = new_block(l);
    body = new_block(l);
    l->loop_depth--;
    ...
    l->b = c->head;
    more = temp(l, ir_binary(l->f, l->b, IR_ULT, IR_I8, temp(l, c->index),
...
static void lanes_end(struct lowerer *l, struct lanes *c)
{
    ...
    next = temp(l, ir_binary(l->f, l->b, IR_ADD, IR_I64, temp(l, c->index),
                             ir_int_op(IR_I64, 1)));
                                 ...
                                 size_t k)
{
    ...
    if (!c->loop) {
        return offset_address(l, base, field_offset(l, t, &t->fields[k].name));
    }
    index = k == 0 ? temp(l, c->index)
                   : temp(l, ir_binary(l->f, l->b, IR_ADD, IR_I64,
                                       temp(l, c->index),
                                       ir_int_op(IR_I64, k)));
    offset = temp(l, ir_binary(l->f, l->b, IR_MUL, IR_I64, index,
                               size_operand(l, type_simd_lane(t))));
    return temp(l, ir_ptradd(l->f, l->b, base, offset));
}
...
static struct ir_operand read_lane(struct lowerer *l, const struct type *lane,
                                   struct ir_operand at)
{
    struct ir_operand v = temp(l, ir_load(l->f, l->b, ir_type_of(lane), at));

    if (lane->kind == TYPE_F16) {
        v = temp(l, ir_unary(l->f, l->b, IR_HEXT, IR_F32, v));
    }
    return v;
}
...
{
    if (lane->kind == TYPE_F16) {
        v = temp(l, ir_unary(l->f, l->b, IR_HTRUNC, IR_I16, v));
    }
    ir_store(l->f, l->b, ir_type_of(lane), v, at);
}
                                           ...
                                           const struct expr *e)
{
    ...
    for (k = 0; k < c.count && !l->failed; k++) {
        ...
        struct ir_operand r;
        if (checked) {
            binary_checks(l, op, lane, a, b, e->pos.line);
        }
        r = temp(l, ir_binary(l->f, l->b, simd_lane_op(op, lane),
                              is_comparison(op) ? IR_I8 : lane_math(lane), a,
                              b));
        if (is_comparison(op)) {
                                       ...
                                       struct ir_operand index, size_t count)
{
    ...
    const struct ir_global *text =
        check_text(l, e->pos.line, "index out of bounds");
    struct ir_operand length = slice_length(l, slice, slice_expr->type);
    struct ir_operand base = temp(l, ir_load(l->f, l->b, IR_PTR, slice));
    struct ir_operand ok =
        temp(l, ir_binary(l->f, l->b, IR_ULT, IR_I8, index, length));
    struct ir_operand offset;

    check_branch(l, ok, false, text, CHECK_BOUNDS, index, length, NULL);
    if (count > 1) {
        struct ir_operand room =
            temp(l, ir_binary(l->f, l->b, IR_SUB, IR_I64, length, index));
        struct ir_operand last =
            temp(l, ir_binary(l->f, l->b, IR_ADD, IR_I64, index,
                              ir_int_op(IR_I64, count - 1)));
        ok = temp(l, ir_binary(l->f, l->b, IR_ULT, IR_I8,
                               ir_int_op(IR_I64, count - 1), room));
        check_branch(l, ok, false, text, CHECK_BOUNDS, last, length, NULL);
    }
    offset = temp(l, ir_binary(l->f, l->b, IR_MUL, IR_I64, index,
                               size_operand(l, lane)));
    return temp(l, ir_ptradd(l->f, l->b, base, offset));
                                    ...
                                    struct ir_operand y)
{
    ...
    text_free(&name);
    scratch = temp(l, ir_entry_slot(l->f, ir_aggregate(agg)));
    lanes_begin(l, &c, t->field_count, true);
    {
        ...
        if (y.kind != IR_NONE) {
            ...
            v = temp(l, ir_binary(l->f, l->b, fold_op(SIMD_OP_SUM, lane) == IR_FADD
                                                  ? IR_FMUL
                                                  : IR_MUL,
                                  math, v, w));
        }
        to = temp(l, ir_ptradd(l->f, l->b, scratch,
                               temp(l, ir_binary(l->f, l->b, IR_MUL, IR_I64,
                                                 temp(l, c.index),
                                                 ir_sym_operand(l->m,
                                                     ir_sym_size_of(l->m,
    ...
    for (half = t->field_count / 2; half >= 1; half /= 2) {
        ...
        low_at = temp(l, ir_ptradd(l->f, l->b, scratch,
                                   temp(l, ir_binary(l->f, l->b, IR_MUL,
                                                     IR_I64, temp(l, c.index),
                                                     size))));
        high_at = temp(l, ir_ptradd(l->f, l->b, low_at,
                                    temp(l, ir_binary(l->f, l->b, IR_MUL,
                                                      IR_I64,
                                                      ir_int_op(IR_I64, half),
                                                      size))));
        low = temp(l, ir_load(l->f, l->b, math, low_at));
        high = temp(l, ir_load(l->f, l->b, math, high_at));
        if (op == SIMD_OP_SUM || op == SIMD_OP_DOT) {
            ir_store(l->f, l->b, math,
                     temp(l, ir_binary(l->f, l->b, fold, math, low, high)),
                     low_at);
        } else {
            struct ir_block *take = new_block(l);
            struct ir_block *keep = new_block(l);
            ir_branch(l->f, l->b,
                      temp(l, ir_binary(l->f, l->b, fold, IR_I8, high, low)),
                      take, keep);
            l->b = take;
    ...
    result = temp(l, ir_load(l->f, l->b, math, scratch));
    /* The value of f16 lanes goes back to an f16, which reads as an f32. */
    if (lane->kind == TYPE_F16) {
        result = temp(l, ir_unary(l->f, l->b, IR_HTRUNC, IR_I16, result));
        result = temp(l, ir_unary(l->f, l->b, IR_HEXT, IR_F32, result));
    }
    return result;
}
...
static struct ir_operand lower_simd(struct lowerer *l, const struct expr *e)
{
    ...
    enum ir_type bits = ir_type_of(lane);
    ...
    case SIMD_OP_SHUFFLE:
        ...
        if (!loops) {
            ir_vshuffle(l->f, l->b, bits, dst, x, e->as.simd.lanes,
                        t->field_count, agg_of(l, t));
            return dst;
        }
        lanes_begin(l, &c, t->field_count, false);
        for (k = 0; k < c.count; k++) {
            struct ir_operand v = temp(l, ir_load(l->f, l->b, bits,
                                                  lane_at(l, &c, t, x,
                                                          e->as.simd.lanes[k])));
    ...
    case SIMD_OP_MAX:
        ...
        return temp(l, ir_vreduce(l->f, l->b, fold_op(e->as.simd.op, lane),
                                  bits, x, agg_of(l, t)));
    case SIMD_OP_DOT:
        x = simd_value(l, args[0]);
        ...
        ir_vbinary(l->f, l->b, lane_math(lane) == IR_F32 ||
                                   ...
                                   ? IR_FMUL
                                   : IR_MUL,
                   bits, dst, x, y, agg_of(l, t));
        return temp(l, ir_vreduce(l->f, l->b, fold_op(SIMD_OP_SUM, lane), bits,
                                  dst, agg_of(l, t)));
    case SIMD_OP_SELECT: {
           ...
           the bits of a lane, which chooses without a branch. */
        {
            ...
            struct ir_operand r;
            lanes_begin(l, &c, t->field_count, true);
            m = temp(l, ir_load(l->f, l->b, IR_I8,
                                lane_at(l, &c, mask, z, 0)));
            if (word != IR_I8) {
                m = temp(l, ir_unary(l->f, l->b, IR_ZEXT, word, m));
            }
            m = temp(l, ir_unary(l->f, l->b, IR_NEG, word, m));
            a = temp(l, ir_load(l->f, l->b, word, lane_at(l, &c, t, x, 0)));
            b = temp(l, ir_load(l->f, l->b, word, lane_at(l, &c, t, y, 0)));
            a = temp(l, ir_binary(l->f, l->b, IR_AND, word, a, m));
            m = temp(l, ir_unary(l->f, l->b, IR_NOT, word, m));
            b = temp(l, ir_binary(l->f, l->b, IR_AND, word, b, m));
            r = temp(l, ir_binary(l->f, l->b, IR_OR, word, a, b));
    ...
    default: {
        ...
        lanes_begin(l, &c, t->field_count, true);
        y = temp(l, ir_load(l->f, l->b, IR_I8, lane_at(l, &c, t, x, 0)));
        ir_assign(l->f, l->b, acc,
                  temp(l, ir_binary(l->f, l->b, fold, IR_I8, temp(l, acc), y)));
        lanes_end(l, &c);
        return temp(l, acc);
    }
    }
