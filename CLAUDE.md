...
- 592 ctest tests pass on the development Mac and none is skipped. The ASan and
  the UBSan builds run 591 each, without the `no_paths` test, which needs a
...
- Of the small things, `switch` on a `str` is built, a chain of calls of
  `anti.text.equal`, and `x in lo..hi`. See "Small things" in
  `docs/decisions.md`.
