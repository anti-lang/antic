...
- `keys/private/release-key.pem` holds the private key that signs `SHA256SUMS`. `.gitignore` excludes the directory, and a run stops before the tag when the file is missing.
