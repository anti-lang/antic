...
Built: the operators and the hooks `iter`, `next`, `value`, `index`, `set_index` and `hash`, with any number of indices. `x.hash()` gives a `u64` on every type: a scalar through the finalizer of MurmurHash3 with zero of either sign as one value, a `str` by its bytes, a pointer by its address, and a struct, a tuple, a variant, a `?T`, an array and a slice through the default over their parts in order. A part with a hash of its own is hashed by it, and a class takes the `hash` of `Object` over its field list. The default `==` of a struct and of a class value is built, with each field by its kind in the compiler and in the runtime, and `tests/programs/eq_default.anti`, `eq_kinds.anti` and `tests/errors/equality.anti` hold it and its refusals. `tests/programs/hash_builtin.anti`, `hash_default.anti` and `hash_generic.anti` run in both modes, `tests/modules/hashing/` hashes the types of a library file, and `tests/errors/hashing.anti` holds the refusals.
