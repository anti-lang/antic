...
Built: the checks, with `--checks` and `--no-checks`, `-g`, which writes the line of every statement and keeps the debug sections of the link, the build id in `anti_licenses` of every executable and shared library, the backtraces, with `StackTrace`, `anti.debug.backtrace` and `--anti.backtrace`, and the CPU levels, with `--cpu`, the start-up check and the runtime archive with one runtime per target and level. `symbolize` names the function of a frame in every build and its file and line in a `-g` build. `trace` and the options that decide it are built, and "Hooks and tracing" above names them. The symbols archive of `anti build --release` is built: `<program>-symbols.zip` beside the program, with the same link with its debug sections kept, the map of the program's functions and, on Windows, the PDB. Not built yet: the variables of `-g`, and `anti symbols inventory`, `check` and `resolve`. Windows has not run a trace.
