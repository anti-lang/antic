...
- `types_body_table` in `src/antic/types.c` says which tables a `concrete fn` fills. The checker, lowering and the header read the tables through `types_primary_member`, `types_interface_member` and `types_holds_entry`, so the three agree. A use of a name on a value or a type goes through `reached_member`, which passes over the bodies qualified by an interface. The check that every abstract function is filled asks the same functions, so a body qualified by an interface no longer counts for the primary table or for another interface.
