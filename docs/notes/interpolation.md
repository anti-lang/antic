...
- `anti.text` writes an integer in Anti and a float through `anti_rt_builder_float` of `rt/text.c`, which asks the C library for the digits and lays them out itself, so every target writes the same text. `anti_rt_builder_fill` widens a field in place after the value is written.
