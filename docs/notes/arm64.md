...
- `addr` is `adrp` and `add` with `:lo12:`. The dumps print this ELF form for every ARM64 target. `emitter.md` writes `@PAGE` and `@PAGEOFF` for Mach-O, which llvm-mc requires there.
