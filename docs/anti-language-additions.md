...
- A tuple has a runtime description as well. A tuple field of a class is therefore compared, hashed, serialized, deserialized and reflected as a tuple part of a struct is. `serialize` writes an array field's elements as a JSON array, and `deserialize` reads it back and fails when the count differs from the array's length.
