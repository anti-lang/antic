...
Operator functions at module level may share a name when the types of their first parameters differ, and the compiler picks one by the type of the left operand. One module then gives `Point` and `Rect` each its own `==`. A bare call of a shared name, `eq(a, b)`, picks the function by the type of its first argument, as `a == b` does. This holds for `operator fn` alone, and ordinary functions keep one function per name.
...
- The default `eq` and `hash` of a struct or class take each field by its kind, so the two always agree: a `str` by its bytes, an `own []T` element by element where `T` has `==`, a plain slice or a pointer by address, and every other field as the rules above give it. The runtime's `equals` and `hash` of `Object` follow the same rule, and walk an inline struct, class or array field too, so a class and a struct with the same fields compare alike. A union field cannot be compared: a class with one gets no default `==`, and the refusal names the field, as for a struct.
- A tuple has a runtime description as well. A tuple field of a class is therefore compared, hashed, serialized, deserialized and reflected as a tuple part of a struct is. `serialize` writes an array field's elements as a JSON array, and `deserialize` reads it back and fails when the count differs from the array's length. The runtime keeps each level's length of a multi-level array field in its description, so `serialize` writes nested JSON arrays, `[[1, 2], [3, 4]]` for a `[2][2]int`, and `deserialize` checks the count at every level.
- The compiler writes each class's default `==` and hash as code, as it does for structs, so they work in every build, `--no-reflect` included. The runtime's field walk stays for `serialize`, `deserialize` and `reflect` only. Under `--no-reflect`, a call of those is refused at compile time with a message naming `--no-reflect`.
- A field that holds a handle compares by what the handle means. A Mutex and an object lock are left out of `==` and `hash`, since a lock is not data. A channel and an `own fn` compare by identity, the handle or the code and the snapshot address, as a pointer compares. A `Regex` compares by its pattern text and its flags. A `Match` gets no default `==`, and the refusal names the field, as for a union.
- `a == b` on two arrays of the same type compares element by element, as inside a struct.
