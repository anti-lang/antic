...
- `destroy(&c)` runs the same chain without the final free, for an object on the stack or inline in another object. `destroy(p)` of a pointer to a value of any other type tears that value down in place. An owning struct or tuple goes part by part, and a value that owns nothing is left as it is.
