...
- `destroy(&c)` runs the same chain without the final free, for an object on the stack or inline in another object. `destroy(&c)` of a local counts as moving `c` out: the block then skips its teardown, naming `c` again is refused, and a `destroy` in one branch is a move in one branch. `destroy` of a part of a local, `&s.k`, is refused, and the message says to destroy the whole value or to give the part a class with its own teardown. `destroy(p)` through a pointer that the rules of locals do not govern, the memory of an allocator or a deserialized object, is unchanged. `destroy(p)` of a pointer to a value of any other type tears that value down in place. An owning struct or tuple goes part by part, and a value that owns nothing is left as it is.
- Memory goes back to the allocator it came from, and whoever chose the allocator says which one. `destroy(p, from)` runs the destruct chain of `p` and of everything it owns and returns each piece of owned memory to `from`, an `anti.mem.Allocator`, instead of the C library. `delete(p, from)` does the same and also returns the object itself to `from`. `delete(p)` and `destroy(p)` without an allocator mean the C library's `malloc` and `free` on every system, the same as the language's `alloc` and `free`.
...
- An owning struct or tuple follows the same rules. `=` refuses to copy an existing one and `dup(s)` copies it, each part that owns something as `dup` copies it. It moves when it is returned or passed to an `own` parameter, and naming it after a move is refused. `dup` of a struct or a tuple that owns nothing gives its bytes. `dup(x)` of a value of a type parameter copies the value itself in every copy of the generic, so a pointer argument gives its address and never a copy of the object.
...
- `anti.reflect` reads descriptors: `describe(obj) -> ?*byte`, `field_count(d)` and `field(d, index)`, `function_count(d)` and `function(d, index)`, `get(obj, d, index) -> Value`, `set(obj, d, index, value) may fail`, `call(obj, index, args: []Value) -> Value may fail` and `new(name: str) -> ?*Object`, which gives `none` for a class it cannot build. `Value` is a tagged union of the primitive types, `str` and pointers.
